...
function WalletIcon({ name }: { name: string }) {
  // Simple colored circle icons for each wallet
  ...
  const color = colors[name] || "#768390";
...
export function LoginButton() {
  ...
  // If connected via wallet or Google
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="border-[#21262d] bg-[#161b22] max-w-sm">
          ...
          <div className="space-y-2 mt-2">
            ...
            {/* Wallet options */}
            {connectors
              .filter((c) => c.id !== "injected" || connectors.length <= 2)
              .slice(0, 4)
              .map((connector) => (
                <button
                  key={connector.uid}
                ...
                >
                  ...
                  <span className="text-sm text-[#e6edf3]">{connector.name}</span>
                </button>
              ))}
          </div>
        </DialogContent>
